...
class Class2WeightEstimation:
    def __init__(self, mtow, zfw, wg, cl_alpha, cl_cruise, cl_max, s_wing, b, half_sweep, tr, MAC):
        self.mtow = mtow  # kg
        self.zfw = zfw  # not real value
        self.wg = wg  # not real value, gross weight
        self.Lift = mtow * 9.81
        self.rho_sea = 1.225
        self.cl_alpha = cl_alpha  # placeholder + wing
        self.cl_cruise = cl_cruise  # placeholder + wing
        self.cl_max = cl_max# placeholder + wing
        self.s_wing = s_wing # m^2
        self.h_cruise = 31000  # ft, not needed formula but graph
        self.b = b# m , random value as of right now
        self.half_sweep = half_sweep # in radians
        self.tr = tr  # airfoil data
        self.MAC = MAC

    def gustload(self, Lift,cl_cruise,s_wing,cl_max,rho_sea,zfw,cl_alpha,MAC):
        U_ref=56 #f/s
        u_c = 50 *0.3048 #convergense to m/s
        u_b = 66 *0.3048 #convergense to m/s
        u_d = 25 *0.3048 #convergense to m/s
        v_cruise=(2*Lift/(cl_cruise*rho_sea*s_wing))**0.5
        v_dive = 1.25*v_cruise
        v_stall = (2*Lift/(cl_max*rho_sea*s_wing))**0.5*1.94384 #In knots

        mu_g = 2*zfw/(rho_sea*MAC*cl_alpha*9.81)
        k_g = 0.88*mu_g/(5.3+mu_g)
        v_b = v_stall*(1+(k_g*U_ref*v_cruise*1.94384*cl_alpha)/(498*zfw*2.20462/(s_wing*10.7639)))**0.5/1.94384

        delta_n_cruise = rho_sea*v_cruise*cl_alpha*k_g*u_c/(2*zfw/s_wing)
        delta_n_dive = rho_sea*v_dive*cl_alpha*k_g*u_d/(2*zfw/s_wing)
        delta_n_b = rho_sea*v_b*cl_alpha*k_g*u_b/(2*zfw/s_wing)
        n_max = max(delta_n_b,delta_n_cruise,delta_n_dive) +1
        n_ult = 1.5*(n_max)
        return n_max, n_ult, v_dive

    def wingweight(self, b,half_sweep,n_ult,w_g,s_wing):
        b_ref = 1.905
        bs = b/cos(half_sweep)
        k_w = 6.67*10^-3
        wing_weight = w_g * k_w * bs**0.75 *(1+(b_ref/bs)**0.5)*n_ult**0.75 * (bs/tr /(w_g/s_wing))^0.3
        wing_weight= 0.95 * wing_weight    # decrease by five for two engines
        return wing_weight

    def fuselageweight(self,v_dive, wetted_fuselage, h_f, b_f): #fuselage area is wetted calculated for drag
        kwf = 0.23        #CONSTANT
        lt = 30           # distance between quarter chord point of wing and horizontal tail root todo ???FORMULA
        bf = b_f           #max fuselage width
        hf = h_f           #max fuselage height
        fuselage_weight = kwf * (v_dive**lt/(bf+hf))**0.5 *wetted_fuselage **1.2
        fuselage_weight = 1.08 * fuselage_weight   # add 4% if main wheel in fuselage
        return fuselage_weight

    def Tailweight(self, s_h,k_h,lambda_h,lambda_v):
        x_h = s_h**0.2 *v_dive/(cos(lambda_h))**0.5  #todo i dont understand this
        x_v = s_v**0.2 *v_dive/(cos(lambda_v))**0.5  #todo i dont understand this
        f_h = input("y for horizontal tail")         #todo i dont understand this
        f_v = input("y for vertical tail")
        weight_tail_hor = s_h * k_h * f_h
        weight_tail_ver = s_v * k_v * f_v
        weight_tail = weight_tail_hor + weight_tail_ver
        return weight_tail, weight_tail_hor, weight_tail_ver

    def fixedequip(self, zfw):
        weight_fixed_equipment = 0.196*zfw**0.91
        return weight_fixed_equipment

    def zerofuelweight(self, weight_fixed_equipment ,fuselage_weight, wing_weight, weight_tail, weight_l_gear, weight_payload):
        zfw = weight_fixed_equipment + fuselage_weight + wing_weight + weight_tail +weight_l_gear + weight_payload
        return zfw
